...
class OD3D_Meta(ABC):
    ...
    def load_from_meta_with_rfpath(cls, path_meta: Path, rfpath: Path):
        return cls(
            **cls.load_omega_conf_with_rfpath(path_meta=path_meta, rfpath=rfpath),
        )
    ...
    @classmethod
    def complete_nested_metas(
        cls,
        path_meta: Path,
        dict_nested_metas: Union[Dict, DictConfig, None],
        parent_key="",
        separator="/",
        dict_nested_metas_ban: Union[Dict, DictConfig, None] = None,
    ):
        ...
        for key, value in dict_nested_metas.items():
            ...
            if value is None:
                ...
                if dir_fpaths[0].is_dir():
                    if (
                        dict_nested_metas_ban is None
                        or key not in dict_nested_metas_ban
                    ):
                        dict_nested_frames_completed[key] = cls.complete_nested_metas(
                            path_meta=path_meta,
                            parent_key=new_key,
                            dict_nested_metas={
                                f"{dir_fpath.stem}": None for dir_fpath in dir_fpaths
                            },
                        )
                    ...
                    if (
                        ...
                        dict_nested_frames_completed[key] = [
                            dir_fpath.stem
                            for dir_fpath in sorted(
                                dir_fpaths,
                                key=lambda f: [
                                    OD3D_Meta.atoi(val)
                                    for val in re.split(r"(\d+)", f.stem)
                                ],
                            )
                        ]
                    elif dict_nested_metas_ban[key] is not None:
                        dict_nested_frames_completed[key] = [
                            ...
                            if dir_fpath.stem not in dict_nested_metas_ban[key]
                        ]
                    else:
                        pass
                ...
                if dict_nested_metas_ban is None or key not in dict_nested_metas_ban:
                    ...
                    dict_nested_frames_completed[key] = [
                        val for val in value if val not in dict_nested_metas_ban[key]
                    ]
                else:
                    pass

        if (
            len(dict_nested_frames_completed.keys()) == 1
            and "" in dict_nested_frames_completed.keys()
        ):
    ...
    def unroll_nested_metas(dict_nested_meta: Dict, separator="/"):
        ...
        for key, frames_names in dict_frames.items():
            for frame_name in frames_names:
                frame_name_unique = (
                    f"{key}{separator}{frame_name}" if key else frame_name
                )
