...
class MonoLMB(OD3D_SequenceDataset):
    ...
    def setup(config: DictConfig):
        ...
        for fpath_video in fpaths_videos:
            path_video_frames = path_frames.joinpath(
                fpath_video.relative_to(path_videos).with_suffix(""),
            )
    ...
    def extract_meta(config: DictConfig):
        ...
        for category in categories:
            ...
            if sequences_names is None and (
                ...
                sequences_names = [
                    fpath.stem for fpath in path_sequences.joinpath(category).iterdir()
                ]
            if (
            ...
            for sequence_name in sequences_names:
                ...
                for fpath_frame in path_sequences.joinpath(
                    sequence_meta.name_unique,
                ).iterdir():
                    import torch
