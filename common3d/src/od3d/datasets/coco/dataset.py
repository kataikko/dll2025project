...
from od3d.datasets.coco.enum import COCO_CATEGORIES
...
class COCO(OD3D_Dataset):
    ...
    def filter_list_frames_unique(self, list_frames_unique):
        ...
        for frame_name_unique in tqdm(list_frames_unique):
            meta = COCO_FrameMeta.load_from_meta_with_name_unique(
    ...
    def get_item(self, item):
        frame_meta = COCO_FrameMeta.load_from_meta_with_name_unique(
            path_meta=self.path_meta,
    ...
    def setup(config: DictConfig):
        ...
        for subset in subsets_images:
            ...
            if path_subset.exists() and not config.setup.override:
                ...
                run_cmd(
                    f"cd {path_raw} && wget http://images.cocodataset.org/zips/{subset}.zip",
                    logger=logger,
                    live=True,
                )
                run_cmd(
                    f"cd {path_raw} && unzip {subset}.zip",
                    logger=logger,
                    live=True,
                )
        ...
        for subset in subsets_annotations:
            ...
            if path_subset.exists() and not config.setup.override:
                ...
                path_raw.mkdir(parents=True, exist_ok=True)
                run_cmd(
                    f"cd {path_raw} && wget http://images.cocodataset.org/annotations/{subset}.zip",
                    logger=logger,
                    live=True,
                )
                run_cmd(
    ...
    def extract_meta(config: DictConfig):
        ...
        for subset in ["val2017", "train2017"]:  # 'train2017', 'val2017'
            annFile = path_raw.joinpath(
                "annotations",
                f"instances_{subset}.json",
            )  # f'{dataDir}/annotations/instances_{dataType}.json'
            ...
            for i in tqdm(range(len(images))):
                ...
                frame_meta = COCO_FrameMeta.load_from_raw(
                    image=image,
                    subset=subset,
                    annotations=annotations,
                    dict_category_id_to_name=dict_category_id_to_name,
                )
