...
from od3d.datasets.imagenet.enum import (
    IMAGENET_CATEOGORIES,
)
...
class ImageNet(OD3D_Dataset):
    def __init__(
        ...
        for co3d_cat in co3d_cats:
            map_categories_co3d_to_imagenet[co3d_cat] = list(
                filter(lambda cat: co3d_cat in cat, imagenet_cats),
            )
        from od3d.datasets.co3d.enum import CO3D_CATEGORIES
    ...
    def get_item(self, item):
        frame_meta = ImageNetFrameMeta.load_from_meta_with_name_unique(
    ...
    def setup(config: DictConfig):
        ...
        od3d.io.write_config_to_json_file(
            config=config.credentials.kaggle,
            fpath=Path("~/.kaggle/kaggle.json"),
        )
        od3d.io.run_cmd(
            f"cd {path_raw} && kaggle competitions download -c imagenet-object-localization-challenge",
            logger=logger,
            live=True,
        )
        logger.info("extracting files...")
        od3d.io.unzip(
            path_raw.joinpath("imagenet-object-localization-challenge.zip"),
            dst=path_raw,
        )
    ...
    def extract_meta(config: DictConfig):
        ...
        categories = od3d.io.read_str_from_file(
            path_raw.joinpath("LOC_synset_mapping.txt"),
        ).split("\n")
        ...
        for subset in ["train_cls", "train_loc", "val", "test"]:  # # 'test'
            ...
            if "test" in subset:
                ...
                annots = sorted(
                    od3d.io.read_str_from_file(
                        path_raw.joinpath("LOC_train_solution.csv"),
                    ).split("\n")[1:],
                )
            else:
                annots = sorted(
                    od3d.io.read_str_from_file(
                        path_raw.joinpath("LOC_val_solution.csv"),
                    ).split("\n")[1:],
                )
            path_meta_subset = OD3D_FrameMeta.get_path_metas(
                path_meta=path_meta,
            ).joinpath(subset)
            if not path_meta_subset.exists() or config.get("extract_meta", False).get(
                "override",
                False,
            ):
                fnames_lines = od3d.io.read_str_from_file(
                    path_raw.joinpath(
                        "ILSVRC",
                        "ImageSets",
                        "CLS-LOC",
                ...
                fnames_lines = sorted(
                    fnames_lines,
                    key=lambda fname_line: fname_line.split(",")[0],
                )  # .split('_')[1]
                for i in tqdm(range(len(fnames_lines))):
                    ...
                    if "test" in subset:
                        category = "None"
                        l_bbox = torch.Tensor([0.0, 0.0, 0.0, 0.0]).tolist()
                        rfpath_rgb = Path("ILSVRC").joinpath(
                            "Data",
                            "CLS-LOC",
                            "test",
                            f"{rfpath}.JPEG",
                        ...
                        if subset == "train_cls":
                            ...
                            if not annots[i].startswith(fname):
                                logger.warning(
                                    f"skip frame {fname} as annotation and fname dont match: {annots[i]} {fname}",
                                )
                                continue
                            l_bbox = torch.LongTensor(
                                [int(a) for a in annots[i].split(" ")[1:5]],
                            ).tolist()
                        fpath_annot = path_raw.joinpath(
                            "ILSVRC",
                            "Annotations",
                            "CLS-LOC",
                            "train",
                        ...
                        l_bbox = torch.LongTensor(
                            [int(a) for a in annot_split[1:5]],
                        ).tolist()
                        fpath_annot = path_raw.joinpath(
                            "ILSVRC",
                            "Annotations",
