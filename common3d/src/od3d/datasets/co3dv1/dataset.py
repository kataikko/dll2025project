...
from od3d.datasets.dataset import (
...
class CO3Dv1(CO3D):
    ...
    def setup(config: DictConfig):
        ...
        if path_raw.exists() and not config.setup.override:
            ...
            run_cmd(
                cmd=f"cd {path_raw}/co3d && git checkout v1",
                live=True,
                logger=logger,
            )
            ...
            run_cmd(
                cmd=f'python {path_co3d_repo.joinpath("download_dataset.py")} --download_folder {path_raw}',
                live=True,
                logger=logger,
            )
