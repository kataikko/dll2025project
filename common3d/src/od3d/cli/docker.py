...
@app.command()
def zsp(no_cache: bool = typer.Option(False, "-n", "--no-cache")):
    logging.basicConfig(level=logging.INFO)
    if no_cache:
        cmd = "docker build -f third_party/envs/Zero-Shot-Pose/Dockerfile -t limpbot/zsp:v1 third_party/zero-shot-pose --no-cache"
    else:
