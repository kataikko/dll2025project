...
class ZSP(OD3D_Method):
    ...
    def train(
        ...
        for cat_id, category in enumerate(categories):
            ...
            for r, ref_mesh_id in enumerate(ref_mesh_ids):
                ...
                transform_low_res = OD3D_Transform.create_by_name(
                    "scale_mask_separate_1_centerzoom224",
                )
                transform_high_res = OD3D_Transform.create_by_name(
                    "scale_mask_separate_1_centerzoom224",
                )
                transform_rescale = 1.0

                logger.info(f"ref {r+1} out of {len(ref_mesh_ids)}")
                ref_frames_N = len(ref_sequences[ref_mesh_id].frames_names)
                ref_frames_S = 10
                ref_frames_indices = np.linspace(
                    0,
                    ref_frames_N - 1,
                    ref_frames_S,
                ).astype(int)
                ref_frames_low_res = [
                    transform_low_res(
                        ref_sequences[ref_mesh_id].get_frame_by_index(ref_frame_index),
                    )
                    for ref_frame_index in ref_frames_indices
                ]
                ref_frames_high_res = [
                    transform_high_res(
                        ref_sequences[ref_mesh_id].get_frame_by_index(ref_frame_index),
                ...
                srcs_frames_N = [
                    len(src_sequences[src_mesh_id].frames_names)
                ...
                src_frames_low_res = [
                    transform_low_res(
                        src_sequences[src_mesh_id].get_frame_by_index(
                            srcs_frames_ids[s],
                        ),
                    )
                ...
                ref_sequence_scale = torch.Tensor([1.0])[None,]
                ...
                src_cam_source = None
                src_sequences_scales = torch.Tensor(
                    [1.0 for s, src_mesh_id in enumerate(src_mesh_ids)],
                )

                # data = {'img': batch.rgb, 'cam_tform4x4_obj': batch.cam_tform4x4_obj}
                # src_H, src_W =
                ref_image = torch.stack(
                    [src_frame.get_rgb() for src_frame in src_frames_low_res],
                    dim=0,
                )
                B = len(ref_image)
                ref_scalings = src_frames_low_res[
                    0
                ].size.clone()  # torch.stack([src_frame.size], dim=0)
                ref_scalings[:] = transform_rescale  # * src_frame.depth_mask
                ref_depth_map = torch.stack(
                    [
                        src_frame.get_depth()
                        for s, src_frame in enumerate(src_frames_high_res)
                    ],
                    dim=0,
                )
                # ref_depth_map[ref_depth_map > ref_depth_map.flatten(2).mean(dim=-1)[..., None, None] * 2] = 0

                ref_cam_intr = torch.stack(
                    [src_frame.get_cam_intr4x4() for src_frame in src_frames_high_res],
                    dim=0,
                )
                ref_cam_extr = torch.stack(
                    [
                        src_frame.get_cam_tform4x4_obj(src_cam_source)
                        for src_frame in src_frames_high_res
                    ],
                    dim=0,
                )
                ref_cam_extr[:, :3, 3] /= src_sequences_scales[:, None]

                all_target_images = torch.stack(
                    [ref_frame.get_rgb() for ref_frame in ref_frames_low_res],
                    dim=0,
                )[
                    None,
                ].repeat(
                ...
                target_scalings = ref_frames_low_res[
                    0
                ].size.clone()  # torch.stack([ref_frame.size for ref_frame in ref_frames], dim=0)[None,]
                target_scalings[:] = transform_rescale  # * ref_frame.depth_mask
                target_depth_map = torch.stack(
                    [
                        ref_frame.get_depth()
                        for r, ref_frame in enumerate(ref_frames_high_res)
                    ],
                    dim=0,
                )[
                    None,
                ...
                target_cam_intr = torch.stack(
                    [ref_frame.get_cam_intr4x4() for ref_frame in ref_frames_high_res],
                    dim=0,
                )[
                    None,
                ].repeat(
                ...
                target_cam_extr = torch.stack(
                    [
                        ref_frame.get_cam_tform4x4_obj(ref_cam_source)
                        for ref_frame in ref_frames_high_res
                    ],
                    dim=0,
                )[
                    None,
                ...
                if self.config.use_train_only_to_collect_target_data:
                    ...
                    target_cam_extr = torch.stack(
                        [
                            ref_frame.get_cam_tform4x4_obj()
                            for ref_frame in ref_frames_high_res
                        ],
                        dim=0,
                    )[
                ...
                try:
                    resp = requests.post(
                        f"http://127.0.0.1:{self.docker_port}/predict",
                        files={"file": bytes_io},
                    )
                    # all_pred_ref_tform_src = resp.json()['obj2_tform_obj1']
                    pred_ref_tform_srcs = resp.json()["obj2_tform_obj1"]
                    pred_ref_tform_srcs = torch.Tensor(pred_ref_tform_srcs).to(
                        device=self.device,
                    )
                    ...
                    logger.info(e)
                    pred_ref_tform_srcs = (
                        torch.eye(4)[None,].repeat(B, 1, 1).to(device=self.device) * 0.0
                    )
                ...
                for s, src_mesh_id in enumerate(src_mesh_ids):
                    ...
                    gt_ref_tform_src = tform4x4(
                        inv_tform4x4(ref_labeled_obj_tform_obj),
                        src_labeled_obj_tform_obj,
                    )

                    if self.config.aligned_name is not None:
                        aligned_name = f"{self.config.aligned_name}/r{r}"
                        aligned_cuboid_tform_obj = tform4x4(
                            ref_labeled_obj_tform_obj,
                            pred_ref_tform_src,
                        )
                        src_sequences[src_mesh_id].write_aligned_mesh_and_tform_obj(
                            mesh=ref_sequences[ref_mesh_id].read_mesh(),
    ...
    def inference_batch(self, batch: OD3D_Frames):
        ...
        for r in range(len(self.target_data[0])):
            ...
            for b in range(B):
                categorical_target_data = self.target_data[batch.category_id[b].item()][
                    r
                ]
                all_target_images.append(
                    categorical_target_data["all_target_images"],
                )  # [None,].repeat(B, 1, 1, 1, 1)
                target_depth_map.append(
                    categorical_target_data["target_depth_map"],
                )  # [None,].repeat(B, 1, 1, 1, 1)
                target_cam_extr.append(
                    categorical_target_data["target_cam_extr"],
                )  # [None,].repeat(B, 1, 1, 1)
                target_cam_intr.append(
                    categorical_target_data["target_cam_intr"],
                )  # [None,].repeat(B, 1, 1, 1)
            ...
            cam_tform4x4_obj = tform4x4(
                ...
                inv_tform4x4(pred_ref_tform_srcs),
            )
        ...
        for b in range(B):
            ...
            if f"{prefix}rot_diff_rad" not in results.keys():
                results[f"{prefix}rot_diff_rad"] = all_rot_diff_rad.T[b][
                    None,
                ]  # [] torch.stack(all_rot_diff_rad, dim=0).permute(1, 0)
            else:
                results[f"{prefix}rot_diff_rad"] = torch.cat(
                    [results[f"{prefix}rot_diff_rad"], all_rot_diff_rad.T[b][None,]],
                    dim=0,
                )
        ...
        # results['sim'] = sim
        results["cam_tform4x4_obj"] = all_cam_tform4x4_obj[
            0
        ]  #  random.choice(all_cam_tform4x4_obj)
