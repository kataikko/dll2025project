...
def einsum_cdist(comb, featsA, featsB):
    ...
    removed_subscripts = list(
        set(featsA_input_subscripts + featsB_input_subscripts) - {*output_subscripts},
    )
    ...
    batch_subscripts = list(
        {*input_subscripts[0]}.intersection({*input_subscripts[1]})
        - {removed_subscript},
    )
    featsA_specific_subscripts = list(
        {*input_subscripts[0]} - {*batch_subscripts} - {removed_subscript},
    )
    featsB_specific_subscripts = list(
        {*input_subscripts[1]} - {*batch_subscripts} - {removed_subscript},
    )
    ...
    featsA_intermed_subscripts = "".join(
        batch_subscripts + featsA_specific_subscripts + [removed_subscript],
    )
    featsB_intermed_subscripts = "".join(
        batch_subscripts + featsB_specific_subscripts + [removed_subscript],
    )
    ...
    featsA_bAd = torch.einsum(
        featsA_input_subscripts + "->" + featsA_intermed_subscripts,
        featsA,
    )
    featsB_bBd = torch.einsum(
        featsB_input_subscripts + "->" + featsB_intermed_subscripts,
        featsB,
    )
    ...
    dist = torch.cdist(
        featsA_bAd.reshape(
            batch_shapes.numel(),
            featsA_specific_shapes.numel(),
            feats_dim,
        ),
        featsB_bBd.reshape(
            batch_shapes.numel(),
            featsB_specific_shapes.numel(),
            feats_dim,
        ),
    )
    dist = dist.reshape(batch_shapes + featsA_specific_shapes + featsB_specific_shapes)

    dist_intermed_subscripts = "".join(
        batch_subscripts + featsA_specific_subscripts + featsB_specific_subscripts,
    )
