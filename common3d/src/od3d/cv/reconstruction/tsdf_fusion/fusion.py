...
class TSDFVolume:
    ...
    def integrate(self, color_im, depth_im, cam_intr, cam_pose, obs_weight=1.0):
        ...
        if self.gpu_mode:  # GPU mode: integrate voxel volume (calls CUDA kernel)
            ...
            new_b = np.minimum(
                255.0,
                np.round((w_old * old_b + obs_weight * new_b) / w_new),
            )
            new_g = np.minimum(
                255.0,
                np.round((w_old * old_g + obs_weight * new_g) / w_new),
            )
            new_r = np.minimum(
                255.0,
                np.round((w_old * old_r + obs_weight * new_r) / w_new),
            )
