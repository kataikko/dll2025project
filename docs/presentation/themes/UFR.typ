...
#let slide(
  ...
  let header(self) = {
    ...
    set text(fill: self.colors.neutral-lightest, weight: "bold", size: 1.2em, font: "Rethink Sans")
  ...
  let footer(self) = {
    ...
    set text(size: 0.8em, font: "Rethink Sans")
    pad(
      .5em,
      components.left-and-right(
...
#let title-slide(
  ...
  let body = {
    set text(fill: self.colors.neutral-darkest, font: "Rethink Sans")
    set std.align(horizon)
    block(
      width: 100%,
      inset: 2em,
      {
        components.left-and-right(
          {
            text(size: 1.8em, text(weight: "bold", info.title))
            if info.subtitle != none {
              linebreak()
              text(size: 1.2em, info.subtitle, weight: "bold")
            }
          },
...
#let new-section-slide(config: (:), level: 1, numbered: true, body) = touying-slide-wrapper(self => {
  let slide-body = {
    set std.align(horizon)
    show: pad.with(20%)
    set text(size: 1.5em, font: "Rethink Sans")
    stack(
      dir: ttb,
      spacing: 1em,
